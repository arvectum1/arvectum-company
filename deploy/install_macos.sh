...
TEMPLATE="$REPO_PATH/deploy/com.arvectum.ai-eng-001.plist.template"

if [[ ! -f "$REPO_PATH/ai_workforce/ai_eng_001/cli.py" ]]; then
  echo "ERROR: AI-ENG-001 runtime not found under $REPO_PATH" >&2
  exit 2
fi
...
if ! command -v opencode >/dev/null 2>&1; then
  echo "ERROR: opencode not found. Install/configure OpenCode first, then rerun." >&2
  exit 2
fi
...
if [[ ! -f "$CONFIG_PATH" ]]; then
  python3 - "$REPO_PATH/config/ai-eng-001.example.json" "$CONFIG_PATH" <<'PY'
...
echo "Inbox:  $STATE_DIR/inbox"
echo "Runs:   $STATE_DIR/runs"
echo "Logs:   $LOG_DIR"
echo "Service: com.arvectum.ai-eng-001"
